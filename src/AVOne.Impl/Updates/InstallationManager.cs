...
namespace AVOne.Impl.Updates
{
    ...
    public class InstallationManager : HttpClientHelper, IInstallationManager
    {
        ...
        public async Task<IReadOnlyList<PackageInfo>> GetAvailablePackages(CancellationToken cancellationToken = default)
        {
            ...
            foreach (RepositoryInfo repository in _config.CommonConfiguration.PluginRepositories)
            {
        ...
        {
            var sLength = source.Count - 1;
            var dLength = dest.Count;
            int s = 0, d = 0;
