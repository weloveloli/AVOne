...
namespace AVOne.Impl.Resolvers
{
    ...
    public static class VideoListResolver
    {
        ...
        public static IReadOnlyList<VideoInfo> Resolve(IVideoResolverProvider videoResolver, IReadOnlyList<VideoFileInfo> videoInfos, INamingOptions namingOptions, bool supportMultiVersion = true, bool parseName = true)
        {
            ...
            var stackResult = StackResolver.Resolve(videoResolver, nonExtras, namingOptions).ToList();
        ...
        private static List<VideoInfo> GetVideosGroupedByVersion(List<VideoInfo> videos, INamingOptions namingOptions)
        {
            if (videos.Count == 0)
            {
                return videos;
            ...
            for (var i = 0; i < alternateVersionsLen; i++)
            {
        ...
        private static bool IsEligibleForMultiVersion(ReadOnlySpan<char> folderName, string testFilePath, INamingOptions namingOptions)
        {
