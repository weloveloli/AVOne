...
namespace AVOne.Impl.Plugins
{
    ...
    public class PluginManager : IPluginManager
    {
            ...
            Version appVersion)
        {
            ...
            for (var a = _jsonOptions.Converters.Count - 1; a >= 0; a--)
            {
...
#pragma warning restore CA1031 // Do not catch general exception types
        ...
        internal LocalPlugin LoadManifest(string dir)
        {
            ...
            var versionIndex = dir.LastIndexOf('_');
            if (versionIndex != -1)
            {
                // Get the version number from the filename if possible.
                metafile = Path.GetFileName(dir[..versionIndex]);
                version = Version.TryParse(dir.AsSpan()[(versionIndex + 1)..], out var parsedVersion) ? parsedVersion : _appVersion;
            }
            else
            {
        ...
        private IEnumerable<LocalPlugin> DiscoverPlugins()
        {
            ...
            for (var x = versions.Count - 1; x >= 0; x--)
            {
